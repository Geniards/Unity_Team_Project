...
public class WGH_AreaJudge : MonoBehaviour
{
    ...
    private bool _isInputedDoubleKey;
    ...
    {
        _isInputProcessing = true;                          // 키를 입력 받았는가를 확인할 bool 변수
        _isInputedDoubleKey = false;                        // 동시 입력 처리를 할 2번째 키를 입력받았는가를 확인할 bool 변수
        ...
        else
        {
            ...
            else if(_playerController.IsAir && key == KeyCode.J)
            {
                ...
                _playerRigid.position = new Vector3(_playerController.transform.position.x, _checkBottomPos.y, 0);
            }
            else if(key == KeyCode.J)    
